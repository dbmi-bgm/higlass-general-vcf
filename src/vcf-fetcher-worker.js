...
// We return an empty tile. We get the data from GeneralVcfTrack
...
const renderSegments = (
  ...
  //const t1 = currTime();
  const displayConfiguration = trackOptions.displayConfiguration;
  ...
  const addTriangle = (x, y, width, height, colorIdx, orientation) => {
    const xLeft = x;
    const xRight = xLeft + width;
    const yTop = y;
    const yBottom = y + height;
    ...
    if(orientation === "triangle-north"){
      const xm = (xLeft+xRight)/2;
      const ulIx = addPosition(xm, yTop);
      const urIx = addPosition(xRight, yBottom);
      const llIx = addPosition(xLeft, yBottom);
      addTriangleIxs(ulIx, urIx, llIx);
    }else if(orientation === "triangle-east"){
      const ym = (yBottom+yTop)/2;
      const ulIx = addPosition(xLeft, yTop);
      const urIx = addPosition(xRight, ym);
      const llIx = addPosition(xLeft, yBottom);
      addTriangleIxs(ulIx, urIx, llIx);
    }else if(orientation === "triangle-south"){
      const xm = (xLeft+xRight)/2;
      const ulIx = addPosition(xLeft, yTop);
      const urIx = addPosition(xRight, yTop);
      const llIx = addPosition(xm, yBottom);
      addTriangleIxs(ulIx, urIx, llIx);
    }else if(orientation === "triangle-west"){
      const ym = (yBottom+yTop)/2;
      const ulIx = addPosition(xLeft, ym);
      const urIx = addPosition(xRight, yTop);
      const llIx = addPosition(xRight, yBottom);
      addTriangleIxs(ulIx, urIx, llIx);
    }
  ...
  const addMark = (x, y, width, height, colorIdx, markType) => {
    if(markType === "rect"){
      addRect(x, y, width, height, colorIdx);
    }else if(markType.substring(0, 8) === "triangle"){
      addTriangle(x, y, width, height, colorIdx, markType);
    }else{
      console.error("Mark not supported")
    }
  };
  ...
  segmentList.forEach((segment, j) => {
    ...
    const colorToUse = segment.color;
    segment['yTop'] = segment.row * (segment.markSize + 2) + 1;
    yTop = segment['yTop'];
    addMark(
      xLeft,
      yTop,
